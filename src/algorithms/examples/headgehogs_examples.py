...
class EtfTrader:
    ...
    def calculate_spread(self, basket):
        ...
        try:
            ...
            symbol_order = {s: i for i, s in enumerate(ETF_CONSTITUENT_SYMBOLS)}
            constituents.sort(key=lambda c: symbol_order[c.name])
            const_prices = [const.wall_mid for const in constituents]
            ...
            spread = INITIAL_ETF_PREMIUMS[b_idx]
